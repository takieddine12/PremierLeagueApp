...
class FixturesFragment : Fragment() {
    ...
    private var adapter: FixturesAdapter? = null
        ...
        if (checkConnectivity(requireContext())){
            ...
            leagueViewModel.observeFixtures().observe(viewLifecycleOwner, Observer {
                it?.let {
                    ...
                    fixturesLayoutBinding.fixturesRecyclerView.adapter  = FixturesAdapter(requireActivity(), handleGrouping(fixturesMutableList))
    ...
    private fun scrollToRound(round: Int) {
        ...
        lifecycleScope.launch {
            leagueViewModel.getNextLeagueFixtures(4328, round, "2020-2021").observe(viewLifecycleOwner, Observer {
                                                    ...
                                                    .observe(viewLifecycleOwner,
                                                        Observer { AwayLogoUrl ->
                                                            AwayLogoUrl.teams!!.map {
                                                                val fixtures = Event(HomeLogoUrl, it, event.idAwayTeam, event.idEvent, event.idHomeTeam, event.intAwayScore,
                                                                    event.intHomeScore, event.intRound,
                                                                    event.strAwayGoalDetails, event.strAwayRedCards, event.strAwayTeam, event.strAwayYellowCards,
                                                                    event.strHomeGoalDetails, event.strHomeRedCards, event.strHomeTeam, event.strHomeYellowCards,
                                                                    UtilsClass.convertDate(event.dateEvent!!),UtilsClass.convertHour(event.strTime))
                                                                fixtures.matchRound = round

                                                                fixturesMutableList.clear()
                                                                fixturesMutableList.add(fixtures)
                                                                leagueViewModel.insertFixtures(fixtures)

                                                                fixturesMutableList.sortedWith(compareBy { it.strTime })
