                    ...
                    NetworkStatesHandler.Status.SUCCESS ->{
                        ...
                        if (!it.data?.events.isNullOrEmpty()) {
                            it.data?.events?.map { match ->
                                ...
                                val liveScores = EventTwo(match.strHomeTeam, match.strAwayTeam,
                                    match.strHomeTeamBadge,match.strAwayTeamBadge,match.intHomeScore,match.intAwayScore,
                                    match.strProgress,UtilsClass.convertHour(match.strEventTime!!),UtilsClass.convertDate(match.dateEvent!!),UtilsClass.convertUpdatedDate(match.updated!!),match.strLeague)
                                val emptyList = mutableListOf(liveScores)
                                leagueViewModel.insertLiveScores(emptyList)
